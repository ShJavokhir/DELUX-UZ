<?php
    ...
    if(isset($_POST['mavzu'])){
       $query = "SELECT * FROM u_test_num where id=".$_POST['mavzu']." AND status='ACTIVE'";
      ...
      $_SESSION['ism'] = Filter($_POST['ism']);
      $_SESSION['familiya'] = Filter($_POST['familiya']);
      $_SESSION['mavzu'] = Filter($_POST['mavzu']);
      $start_time = date("d/m/Y")." | ".date("h:i:sa");
      $query = "SELECT * FROM u_test_num where id=".$_POST['mavzu'];
      $result = queryMysql($query);
      $row = mysqli_fetch_assoc($result);
      $mavzu_name = $row['name'];
      $_SESSION['mavzu_name'] = $mavzu_name;
      $query = "INSERT INTO u_user_taken_test(name,surname,test_name,start_time) VALUES('".$_SESSION['ism']."','".$_SESSION['familiya']."','".$mavzu_name."','".$start_time."')";
      $result = queryMysql($query);
      $s = "select count(id) from u_tests where test_num=".$_SESSION['mavzu'];
      $res = queryMysql($s);
...
<div class="container">
  <h3><?php echo "Ism :".$_SESSION['ism']." |  Familiya :".$_SESSION['familiya'] ; ?></h3>
    <form method="POST" id="tekshirish" action="u_check.php">
  ...
  <div class="tab-content">
    ...
    $query = 'select * from u_tests where test_num='.$_SESSION['mavzu'];
      $result = queryMysql($query);
